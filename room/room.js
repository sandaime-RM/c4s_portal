...
onAuthStateChanged(auth, (us) => {
    ...
    get(ref(db, "rooms")).then((snapshot) => {
        ...
        //予約データを１つずつ表示
        Object.keys(roomData).forEach((key, index) => {
            ...
            //表示
            bookList.innerHTML = '<li class="list-group-item" style="cursor: pointer;" onclick="edit('+index+')"><div class="row"><div class="col-4"><div class="fs-5 fw-bold">'+date+'</div><div class="small">'+room.start+'～'+room.end+'</div></div><div class="col-5"><div class="fs-5 fw-bold">'+room.building+'</div><div>'+room.roomNum+'</div></div><div class="col-3 text-'+color+' text-center">'+status+'</div></div></li>' + bookList.innerHTML;
